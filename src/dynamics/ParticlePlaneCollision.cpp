...
void ParticlePlaneCollision::do_solveCollision()
{
    ...
    float d2Plane = std::abs(
            glm::dot(m_particle->getPosition() - (n * m_plane->distanceToOrigin()), n));
    m_particle->setPosition(m_particle->getPosition() - ((d2Plane - m_particle->getRadius())) * n);
    glm::vec3 u = m_particle->getVelocity();
    m_particle->setVelocity(u - ((1 + m_restitution) * glm::dot(u, n) * n));
}
...
bool testParticlePlane(const ParticlePtr &particle, const PlanePtr &plane)
{
   ...
   */
    ...
    return std::abs(
            glm::dot(particle->getPosition() - (plane->normal() * plane->distanceToOrigin()),
                    plane->normal())) <= particle->getRadius();
}
